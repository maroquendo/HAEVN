...
import { parseVideoUrl, fetchVideoMetadata, isValidVideoUrl, getPlatformDisplayName } from '../utils/videoUrlParser';
...
const AddVideoForm: React.FC<AddVideoFormProps> = ({ onAddVideo, onClose, initialData, currentUser, familyMembers }) => {
  ...
  useEffect(() => {
    ...
    debounceTimeoutRef.current = window.setTimeout(async () => {
      ...
      if (parsed.platform !== 'unknown') {
        ...
        setError('');
        try {
          // Use our unified metadata fetcher
          ...
          // Generate description from title if we have one
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidVideoUrl(url)) {
      setError('Please enter a valid video URL from YouTube, Instagram, TikTok, or Twitter.');
      return;
    }
      ...
      <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg ring-1 ring-black/10 dark:ring-white/10 p-6 rounded-2xl shadow-2xl w-full max-w-lg relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <input
              type="text"
              placeholder="Video URL (YouTube, Instagram, TikTok, Twitter)"
              value={url}
              ...
              <div className="flex flex-wrap gap-3">
                {childMembers.map(child => (
                  <button
                    ...
                    className={`flex items-center space-x-2 p-2 rounded-lg transition-all border-2 ${selectedRecipients.includes(child.id)
